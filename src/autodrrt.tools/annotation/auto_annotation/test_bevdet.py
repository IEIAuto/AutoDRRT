...
class DataFormatDump():
    ...
    def is_all_arrived(self):
        ...
        for index, data in  enumerate(self.json_data):
            ...
            bbox_dict = self.result_json_data[val_index]
            bbox = bbox_dict["boxes_3d"]
            ...
            for idx,obj in enumerate(bbox):
                point_3d = [obj[0], obj[1], obj[2]]

                object_position_radar = np.array([obj[0], obj[1], obj[2]], dtype=np.float64)
                object_y_p_r = [obj[6], obj[7], obj[8]]
                r = Rotation.from_euler('zyx',object_y_p_r,degrees=False)
                object_rotation_quaternion = r.as_quat()
                # print(object_rotation_quaternion)
                # object_rotation_quaternion = [obj[6], obj[7], obj[8], obj["Pose"]["orientation"]["w"]]
                object_size = np.array([obj[3], obj[4], obj[5]], dtype=np.float64)
                length = obj[3]
                width = obj[4]
                height = obj[5]
                ...
                center_z = object_position_radar[2] + height * 0.5
                ...
                text = str(bbox_dict["labels_3d"][idx])
                # print(tuple((np_points_image[0,0].astype(int))))
                font_size = 2
                font_color = (0,255,255)
                font_thickness = 1
                size = cv2.getTextSize(text, font, font_size, font_thickness)[0]
                cv2.putText(camera_image, text, tuple((np_points_image[0,0].astype(int))), font, font_size, font_color, font_thickness)
            ...
            # print(file_name)
            cv2.imwrite('bev_test_result/output_' + file_name, camera_image)
    ...
    def ini_data_json(self):
        with open('data.json','r') as json_file:
            self.json_data = json.load(json_file)
            # print("json is \n")
            # print(len(self.json_data))
    def ini_result_json(self):
        with open('test_output_nms_0.2_score_0.2.json','r') as json_file:
            self.result_json_data = json.load(json_file)
            # print("json is \n")
            print(len(self.result_json_data))
            print(self.result_json_data[0])
def main(args=None):
