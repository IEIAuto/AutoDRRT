...
: Filter("RANSACGroundFilter", options)
{
  ...
  RCLCPP_INFO(get_logger(), "RANSACGroundFilterComponent LAUNCHED");
  ...
  height_threshold_ = declare_parameter("height_threshold", 0.1);
...
void RANSACGroundFilterComponent::testCUDA(pcl::PointCloud<pcl::PointXYZ>::Ptr & cloud,pcl::PointIndices::Ptr & output_inliers,
  pcl::ModelCoefficients::Ptr & output_coefficients)
{
  ...
  for(int i = 0; i < nCount; i++)
  {
    ...
    {
    indexV.push_back(i);
    output_inliers->indices.push_back(i);
    }
  }
    // std::cout << "CUDA push_back end"<< std::endl;
  // memcpy(&output_coefficients->values,modelCoefficients,sizeof(float) * 4);
   for(int i = 0; i < 4; i++)
  {
    output_coefficients->values.push_back(modelCoefficients[i]);
  }
  ...
  std::cout << "CUDA modelCoefficients: " << modelCoefficients[0]
    ...
    <<" "<< modelCoefficients[2]
    <<" "<< modelCoefficients[3]
    <<" count" << output_inliers->indices.size()
    << std::endl;
  ...
  // std::cout << "CUDA find points2: " << check2 << std::endl;
  ...
  PointCloud2 & output)
{
  ...
  // RCLCPP_INFO(get_logger(), "downsampled_cloud cost time is %f",stop_watch_ptr_->toc("processing_time",true));
  // // apply ransac
  stop_watch_ptr_->toc("processing_time",true);
  pcl::PointIndices::Ptr inliers(new pcl::PointIndices);
  pcl::ModelCoefficients::Ptr coefficients(new pcl::ModelCoefficients);
  // RCLCPP_INFO(get_logger(), "downsampled_cloud size %d",downsampled_cloud->width * downsampled_cloud->height);
  // applyRANSAC(current_sensor_cloud_ptr, inliers, coefficients);
  // RCLCPP_INFO(get_logger(), "applyRANSAC cost time is %f",stop_watch_ptr_->toc("processing_time",true));
  
  testCUDA(current_sensor_cloud_ptr, inliers, coefficients);
  RCLCPP_INFO(get_logger(), "CUDA cost time is %f",stop_watch_ptr_->toc("processing_time",true));
  RCLCPP_INFO(get_logger(), "inliers size  is %d",inliers->indices.size());
  RCLCPP_INFO(get_logger(), "coefficients values %f,%f,%f,%f",coefficients->values[0],coefficients->values[1],coefficients->values[2],coefficients->values[3]);
...
RCLCPP_INFO(get_logger(), "Vector3d plane_normal time is %f",stop_watch_ptr_->toc("processing_time",true));
  ...
  extractPointsIndices(
    current_sensor_cloud_ptr, *inliers, segment_ground_cloud_ptr, segment_no_ground_cloud_ptr);
  ...
  //   }
  // }
  // RCLCPP_INFO(get_logger(), "extractPointsIndices time is %f",stop_watch_ptr_->toc("processing_time",true));
  sensor_msgs::msg::PointCloud2::SharedPtr no_ground_cloud_msg_ptr(
    new sensor_msgs::msg::PointCloud2);
  pcl::toROSMsg(*segment_no_ground_cloud_ptr, *no_ground_cloud_msg_ptr);
  ...
  RCLCPP_INFO(get_logger(), "post cost time is %f",stop_watch_ptr_->toc("processing_time",true));
  RCLCPP_INFO(get_logger(), "output size is %ld",output.data.size()/output.point_step);
