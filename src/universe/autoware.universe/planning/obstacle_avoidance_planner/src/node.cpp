...
void ObstacleAvoidancePlanner::onPath(const Path::SharedPtr path_ptr)
{
  ...
  auto output_traj_msg = generateTrajectory(planner_data);
  ...
  output_traj_msg.header.stamp.sec = debuger_time.stamp.sec;
  output_traj_msg.header.stamp.nanosec = debuger_time.stamp.nanosec;
  ...
  const PlannerData & planner_data, const std::vector<TrajectoryPoint> & traj_points)
{
  ...
  GET_STAMP_VIA_ENTITY_EXTRA(p.path)
  {  // publish markers
