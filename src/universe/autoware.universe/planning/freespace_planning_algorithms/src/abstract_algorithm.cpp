  ...
  std::vector<IndexXY> & vertex_indexes_2d) const
{
  ...
  const double back = -1.0 * vehicle_shape.base2back + 0.08;
  const double front = vehicle_shape.length - vehicle_shape.base2back + 0.08;
  const double right = -1.0 * vehicle_shape.width / 2.0 - 1.5 * costmap_.info.resolution;
  const double left = vehicle_shape.width / 2.0 + 1.0 * costmap_.info.resolution;
  ...
  for (double x = back; x <= front; x += costmap_.info.resolution / 2) {
    ...
    addIndex2d(x, right, indexes_2d);
  }
  for (double y = right; y <= left; y += costmap_.info.resolution / 2) {
    addIndex2d(front, y, indexes_2d);
    addIndex2d(back, y, indexes_2d);
  }
