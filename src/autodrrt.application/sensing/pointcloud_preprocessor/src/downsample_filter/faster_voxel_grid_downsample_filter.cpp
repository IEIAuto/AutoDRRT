  ...
  const rclcpp::Logger & logger)
{
  ...
  // RCLCPP_INFO(logger, "input size is %d", input->data.size());
  ...
  // RCLCPP_INFO(logger, "voxel_centroid_map size is %d", voxel_centroid_map.size());
  ...
  // RCLCPP_INFO(logger, "output size is %d", output.data.size());
  ...
  const Eigen::Vector3i & min_voxel)
{
  ...
  for (size_t global_offset = 0; global_offset + input->point_step <= input->data.size();
    ...
    if (std::isfinite(point[0]) && std::isfinite(point[1]), std::isfinite(point[2])) {
      ...
      // std::cout << std::to_string(global_offset) << " " << std::to_string(point[0]) << " point1:" << std::to_string(point[1]) << " point2:" << std::to_string(point[2]) << " " << std::endl;
