  ...
  const TransformInfo & transform_info)
{
  ...
  for (const auto & [raw_p, point_walk_id] :
    ...
    float curr_azimuth{};
    float curr_distance{};
    std::memcpy(&ring_idx, &raw_p[ring_offset], sizeof(ring_idx));
    std::memcpy(&curr_azimuth, &raw_p[azimuth_offset], sizeof(curr_azimuth));
    std::memcpy(&curr_distance, &raw_p[distance_offset], sizeof(curr_distance));
  ...
  if (transform_info.need_transform) {
    for (const auto & [raw_p, point_walk_id] : ranges::views::zip(
      ...
      PointXYZI out_point;
      std::memcpy(&out_point, &raw_p, sizeof(PointXYZI));
      ...
      std::memcpy(
        &out_point.intensity, &raw_p[intensity_offset], sizeof(out_point.intensity));
