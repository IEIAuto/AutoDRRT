...
BEVDet::BEVDet(const std::string &config_file, int n_img, std::vector<Eigen::Matrix3f> _cams_intrin, 
               std::vector<Eigen::Quaternion<float>> _cams2ego_rot, std::vector<Eigen::Translation3f> _cams2ego_trans,
               const std::string &imgstage_file, const std::string &bevstage_file): 
...
void BEVDet::MallocDeviceMemory() {
    CHECK_CUDA(cudaMalloc((void**)&src_imgs_dev, N_img * 3 * src_img_h * src_img_w * sizeof(uchar)));
    ...
    std::cout << "bevstage_engine : " << bevstage_engine->getNbBindings() << std::endl;
    ...
    std::cout << "bevstage_buffer end"<< std::endl;
...
void BEVDet::InitViewTransformer() {
    ...
    CHECK_CUDA(cudaMemcpy(ranks_bev_dev, ranks_bev_host, valid_feat_num * sizeof(int), cudaMemcpyHostToDevice));
    CHECK_CUDA(cudaMemcpy(ranks_depth_dev, ranks_depth_host, valid_feat_num * sizeof(int), cudaMemcpyHostToDevice));
    CHECK_CUDA(cudaMemcpy(ranks_feat_dev, ranks_feat_host, valid_feat_num * sizeof(int), cudaMemcpyHostToDevice));
    CHECK_CUDA(cudaMemcpy(interval_starts_dev, interval_starts_host.data(), unique_bev_num * sizeof(int), cudaMemcpyHostToDevice));
    CHECK_CUDA(cudaMemcpy(interval_lengths_dev, interval_lengths_host.data(), unique_bev_num * sizeof(int), cudaMemcpyHostToDevice));
...
int BEVDet::InitEngine(const std::string &imgstage_file, const std::string &bevstage_file) {
    ...
    std::cout << "create TensorRT Execution Context sucess"<< std::endl;
    
    //std::cout << "N_img: "<<N_img<< std::endl;
    //std::cout << "input_img_h: "<<input_img_h<< std::endl;
    //std::cout << "input_img_w: "<<input_img_w<< std::endl;

    // set bindings
    imgstage_context->setBindingDimensions(0, nvinfer1::Dims32{4, {N_img, 3, input_img_h, input_img_w}});
    
    std::cout << "imgstage_context->setBindingDimensions sucess"<< std::endl;

    bevstage_context->setBindingDimensions(0, nvinfer1::Dims32{4, {1, bevpool_channel * (adj_num + 1), bev_h, bev_w}});

    std::cout << "setBindingDimensions sucess"<< std::endl;
...
void BEVDet::GetAdjFrameFeature(const std::string &curr_scene_token, const Eigen::Quaternion<float> &ego2global_rot,
                                const Eigen::Translation3f &ego2global_trans, float* bev_buffer) {
    ...
    for (int i = 0; i < adj_num; i++) {
        ...
        AlignBEVFeature(ego2global_rot, adj_ego2global_rot, ego2global_trans, adj_ego2global_trans, adj_buffer, 
...
void BEVDet::AlignBEVFeature(const Eigen::Quaternion<float> &curr_ego2global_rot, const Eigen::Quaternion<float> &adj_ego2global_rot,
                             const Eigen::Translation3f &curr_ego2global_trans, const Eigen::Translation3f &adj_ego2global_trans,
                             const float* input_bev, float* output_bev, cudaStream_t stream){
    ...
    CHECK_CUDA(cudaMemcpy(transform_dev, Eigen::Matrix3f(currgrid2adjgrid.transpose()).data(), 9 * sizeof(float), cudaMemcpyHostToDevice));
...
int BEVDet::DoInfer(const camsData& cam_data, std::vector<Box> &out_detections, float &cost_time, int idx) {
    ...
    CHECK_CUDA(cudaMemcpy(src_imgs_dev, cam_data.imgs_dev, N_img * src_img_h * src_img_w * 3 * sizeof(uchar), cudaMemcpyDeviceToDevice));
    ...
    bev_pool_v2(bevpool_channel, unique_bev_num, bev_h * bev_w, (float*)imgstage_buffer[imgbuffer_map["depth"]], 
                (float*)imgstage_buffer[imgbuffer_map["images_feat"]], ranks_depth_dev, ranks_feat_dev, ranks_bev_dev,
                interval_starts_dev, interval_lengths_dev, (float*)bevstage_buffer[bevbuffer_map["BEV_feat"]]
                );
