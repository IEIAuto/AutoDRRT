...
class InferTrtLLM(object):
    def __init__(self, llm_engine_pth, tokenizer_pth) -> None:
        ...
        # custom_token = "<waypoint>"  # 替换为你新加的token
        # # if custom_token in tokenizer.get_vocab():
        # #     print(f"{custom_token} id:", tokenizer.convert_tokens_to_ids(custom_token))
        # # else:
        # #     print(f"{custom_token} 不在词表中！")

        # # # 3. 用同一段文本分词
        # test_text = "Hello, world! " + custom_token
    ...
    def image_to_ptuning(self, input_ids, vision_embeded):
        # 主要的工作还是识别图片IMAGE_TOKEN_INDEX的位置，然后将图像特征插入到对应的位置上，实现图像特征和文本特征的融合
        updated_input_ids = []
        current_vocab_size = self.tokenizer.vocab_size + 1
        
        for batch_idx, cur_input_ids in enumerate(input_ids):
            # 统计图片数量，根据IMAGE_TOKEN_INDEX数量来统计
    ...
    def generate(self, input_ids, vision_embeded, img_metas):
        ...
        # 这的输入是没问题的input_ids, vision_embeded。相当于数据预处理和视觉的部分，是没问题的。
        input_ids, prompt_table = self.image_to_ptuning(input_ids, vision_embeded)
        
        
        # input_ids表示的是token后的id。
        # prompt_table 表示的是embedding后的张量
...
class InferTrt(object):
    ...
    def forward(self, img_metas, input_ids, img, lidar2img, intrinsics, timestamp, 
        ...
        end_time = time.time()
        print(f"each sample inference time: {end_time - start_time:.2f} seconds.")
...
def main():
    ...
    print(args.engine_pth)
    print(args.llm_engine_pth)
    print(args.tokenizer_pth)
