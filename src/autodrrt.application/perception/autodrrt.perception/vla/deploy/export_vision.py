...
def parse_args():
    ...
    parser.add_argument('config',help='test config file path', default="./projects/configs/OmniDrive/eva_base_tinyllama.py")
    parser.add_argument('checkpoint', help='checkpoint file', default="/home/liry/swpld/OmniDrive/work_dirs/eva_base_tinyllama/iter_8790.pth")
...
class OmniDriveVisionTrtProxy(torch.nn.Module):
    ...
    def pos_embedding(self, intrinsics, img2lidars):
        ...
        pad_h, pad_w, _ = (512, 512, 3)
        # pad_h, pad_w, _ = (320, 320, 3)
...
def main():
    ...
    # output_onnx_pth = "./320_img_size_scale_model_result/vision_onnx/"+args.config.split("/")[-1].split(".")[0]+".onnx"
    output_onnx_pth = "./img_size_scale_model_result/"+args.config.split("/")[-1].split(".")[0]+".onnx"
    ...
    img = np.ones([1,6,3,512,512]).astype(np.float32)
    # img = np.ones([1,6,3,320,320]).astype(np.float32)
