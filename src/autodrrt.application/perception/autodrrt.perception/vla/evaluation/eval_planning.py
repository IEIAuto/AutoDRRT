    ...
    for data in key_infos['infos']:
        ...
        if os.path.exists(pred_path+data['token']):
            with open(pred_path+data['token'],'r',encoding='utf8')as f:
                ...
                # traj = pred_data[0]['A'][0]  
                # coordinates_matches = re.findall(r'[+-]?\d+(?:\.\d+)?,[+-]?\d+(?:\.\d+)?', traj)
                # coordinates = [tuple(map(float, coord.split(','))) for coord in coordinates_matches]
                # coordinates_array = np.array(coordinates)
                # preds[data['token']] = coordinates_array
...
if __name__ == "__main__":
    ...
    parser.add_argument('--pred_path', type=str, default='/home/liry/swpld/OmniDrive/img_size_scale_model_result/engine_save_path/', help='Path to the prediction results.')
