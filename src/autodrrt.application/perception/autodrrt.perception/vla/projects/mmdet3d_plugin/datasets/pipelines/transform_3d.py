    ...
    def __call__(self, results):
        ...
        if 'gt_planning' in results.keys():
            ...
            if len(planning_traj) == 6:
                ...
                ##  ===modify===
                traj = ';'.join(f"{format_number(point[0], 2)},{format_number(point[1], 2)}" for point in planning_traj)

                # traj = torch.tensor(planning_traj, dtype=torch.float32).flatten()
                traj_tensor = torch.tensor(planning_traj, dtype=torch.float32).flatten()
            else:
                traj_tensor = torch.zeros(12, dtype=torch.float32)
        else:
            print("gt_planning not in results")
        ...
        results['traj_tensor'] = traj_tensor
        
        # results['vlm_labels'] = traj
...
class LoadAnnoatationVQATest():
    ...
    def __call__(self, results):
        ...
        for anno in sources:
            ...
            anno[1]['value'] = 'Here is the planning trajectory <waypoint_ego>'
