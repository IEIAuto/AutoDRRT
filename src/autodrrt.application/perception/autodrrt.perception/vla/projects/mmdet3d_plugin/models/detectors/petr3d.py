...
from ...datasets.utils.constants import IGNORE_INDEX, EGO_WAYPOINT_TOKEN
...
class Petr3D(MVXTwoStageDetector):
    ...
    def __init__(self,
        ...
        from ..dense_heads.llava_llama import add_special_token

        # if use_gen_token:
        ### Add special token for waypoint
        ...
        self.waypoint_decoder = nn.Sequential(
                    ...
                    nn.GELU(),
                    nn.Linear(2048//2, 6*2),
                )
        
        self.waypoints_loss = nn.MSELoss(reduction='none')

        # self.waypoint_decoder = nn.Linear(2048, 12)
    ...
    def forward_pts_train(self,
        ...
        #     vlm_loss, hidden_state = self.lm_head(input_ids=input_ids, attention_mask=vlm_attn_mask, labels=vlm_labels, images=vision_embeded, use_cache=False)
        ...
        if self.with_lm_head:
            vision_embeded = torch.cat([vision_embeded_obj, vision_embeded_map], dim=1)
            vlm_loss, hidden_state = self.lm_head(input_ids=input_ids, attention_mask=vlm_attn_mask, labels=vlm_labels, images=vision_embeded, use_cache=False)
            predicted_actions = self.waypoint_decoder(hidden_state)
            # gt_traj = traj_tensor[0].unsqueeze(0)
            # gt_traj = torch.tensor(traj_tensor, dtype=predicted_actions.dtype, device=predicted_actions.device)

            gt_traj = torch.stack(traj_tensor).to(dtype=predicted_actions.dtype, device=predicted_actions.device)

            assert gt_traj.shape == predicted_actions.shape, \
                f"Shape mismatch: gt_traj {gt_traj.shape}, predicted_actions {predicted_actions.shape}"

            # l1_loss = (torch.nn.MSELoss()(gt_traj, predicted_actions))
            l1_loss = (torch.nn.SmoothL1Loss()(gt_traj, predicted_actions))
            l1_loss = torch.nan_to_num(l1_loss)
            losses.update({'l1_loss': l1_loss})
            losses.update(vlm_loss=vlm_loss[0])
            # losses.update({'vlm_loss': loss})
    ...
    def forward_train(self,
        ...
        losses = self.forward_pts_train(gt_bboxes_3d,
                                    ...
                                    depths, input_ids, vlm_labels, traj_tensor, vlm_attn_mask, lane_pts, **data)
        ...
        # if self.with_lm_head and not os.path.exists(self.save_path+img_metas[0]['sample_idx']) :
            ...
            for i, input_ids in enumerate(data['input_ids'][0]):
                ...
                if special_token_inputs: 
                    ...
                    ego_fut_pred = ego_fut_preds.tolist()
            

            formatted_pred = ["Here is the planning trajectory [PT" + ''.join(
                [", (+{:.2f}, {:.2f})".format(x, y) for x, y in ego_fut_pred]
            ) + "]."]

            qa_json = [{
                "Q": "Please provide the planning trajectory for the ego car without reasons.",
                "A": formatted_pred
            }]

            with open(self.save_path+img_metas[0]['sample_idx'], 'w') as file:
                json.dump(qa_json, file, indent=2)
