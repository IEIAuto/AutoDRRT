...
class BEVDet(CenterPoint):
    ...
    def bev_encoder(self, x):
        ...
        x = self.img_bev_encoder_backbone(x) # B x 80 x 128 x 128 -> [B x 160 x 64 x 64, B x 320 x 32 x 32, B x 640 x 16 x 16]
...
class BEVDet4D(BEVDet):
    ...
    def prepare_bev_feat(self, img, rot, tran, intrin, post_rot, post_tran,
                         bda, mlp_input):
        x = self.image_encoder(img)  # b x 6 x 512 x 16 x 44
        bev_feat, depth = self.img_view_transformer(
            [x, rot, tran, intrin, post_rot, post_tran, bda, mlp_input])  # bev_feat: b x 80 x 128 x 128  depth:6 x 59 x 16 x 44
        #if False:
        if self.pre_process:
            bev_feat = self.pre_process_net(bev_feat)[0]  # bev_feat: b x 80 x 128 x 128
    ...
    def prepare_inputs(self, inputs):
        ...
        # print('N = ', N)
        # print ('self.num_frame = ', self.num_frame)
