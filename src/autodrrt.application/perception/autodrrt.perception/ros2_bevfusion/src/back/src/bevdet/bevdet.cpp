...
void BEVDet::MallocDeviceMemory(){
    ...
    CHECK_CUDA(cudaMalloc((void**)&img_bevfeat_half, 1 * 80 * 180 * 180 * sizeof(half)));
    CHECK_CUDA(cudaMalloc((void**)&img_bevfeat_nvhalf, 1 * 80 * 180 * 180 * sizeof(nvtype::half)));
...
int BEVDet::DoInfer_1(const camsData& cam_data, float &cost_time, int idx, cudaStream_t stream_img) {
    ...
    CHECK_CUDA(cudaMemcpyAsync(src_imgs_dev, cam_data.imgs_dev, 
               N_img * src_img_h * src_img_w * 3 * sizeof(uchar), cudaMemcpyDeviceToDevice, stream_img));
    ...
    // [STEP 2] : image stage network forward
    if(!imgstage_context->enqueueV2(imgstage_buffer, stream_img, nullptr)){
        printf("Image stage forward failing!\n");
    }
    ...
    float2half_cuda((float*)bevstage_buffer[bevbuffer_map["BEV_feat"]], (half*)img_bevfeat_half, 1 * 80 * 180 * 180);
    CHECK_CUDA(cudaMemcpyAsync(img_bevfeat_nvhalf, img_bevfeat_half, 1 * 80 * 180 * 180 * sizeof(nvtype::half), cudaMemcpyDeviceToDevice, stream_img));
    CHECK_CUDA(cudaDeviceSynchronize());
    ...
    duration<double> sum_time = bevpool_end - pre_start;
    cost_time = sum_time.count() * 1000;
    printf("[Infer total  ] cost time: %5.3lf ms\n", cost_time);
...
int BEVDet::DoInfer_2(const camsData& cam_data, std::vector<Box> &out_detections, float &cost_time, int idx) {
    ...
    auto align_feat_begin = high_resolution_clock::now();

    // [STEP 4] : align BEV feature

    if(use_adj){
        GetAdjFrameFeature(cam_data.param.scene_token, cam_data.param.ego2global_rot, cam_data.param.ego2global_trans, 
                          (float*)bevstage_buffer[bevbuffer_map["BEV_feat"]]);
        CHECK_CUDA(cudaDeviceSynchronize());
    }
    auto align_feat_end = high_resolution_clock::now();
    ...
    duration<double> bevstage_t;
    if(use_adj){
        align_t = align_feat_end - align_feat_begin;
        bevstage_t = bevstage_end - align_feat_end;
    }
    else{
        bevstage_t = bevstage_end - align_feat_end;
    ...
    duration<double> sum_time = post_end - align_feat_begin;
    cost_time = sum_time.count() * 1000;
    printf("[Infer total  ] cost time: %5.3lf ms\n", cost_time);
...
BEVDet::~BEVDet(){
    ...
    CHECK_CUDA(cudaFree(img_bevfeat_half));
    CHECK_CUDA(cudaFree(img_bevfeat_nvhalf));
