...
int BEVDet::DoInfer_1(const camsData& cam_data, float &cost_time) {
    ...
    CHECK_CUDA(cudaMemcpy(img_bevfeat_nvhalf, img_bevfeat_half, 1 * 80 * 180 * 180 * sizeof(nvtype::half), cudaMemcpyDeviceToDevice));
    CHECK_CUDA(cudaDeviceSynchronize());
...
int BEVDet::DoInfer_2(const camsData& cam_data, std::vector<Box> &out_detections, float &cost_time) {
