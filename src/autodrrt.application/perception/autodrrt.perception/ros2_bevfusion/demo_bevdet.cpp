...
void TestNuscenes(YAML::Node &config){
    ...
    float* depth_host = new float[6 * 118 * 16 * 44];   
    float* images_feat_host = new float[6 * 16 * 44 * 80]; 
    ...
    for(int i = 0; i < nuscenes.size(); i++){
        ...
        std::memset(depth_host, 0, 6 * 118 * 16 * 44);
        std::memset(images_feat_host, 0, 6 * 16 * 44 * 80);

        float time = 0.f;
        bevdet.DoInfer_1(nuscenes.data(i), depth_host, images_feat_host, ego_boxes, time, i);
        if(i != 0){
            sum_time += time;
            cnt++;
        }

        for (size_t i = 0; i < 6 * 118 * 16 * 44; i++) { std::cout << *(depth_host + i) << " "; }
        std::cout << std::endl;
        for (size_t i = 0; i < 6 * 16 * 44 * 80; i++) { std::cout << *(images_feat_host + i) << " "; } 
        std::cout << std::endl;
...
void TestSample(YAML::Node &config){
    ...
    std::cout << "sampleData.imgs_dev end"<< std::endl;
...
int main(int argc, char **argv){
    ...
    std::string config_file = "/home/liheyi/Lidar_AI_Solution/ros2_bevfusion/src/ROS2-CUDA-BEVFusion/configure.yaml"; 
