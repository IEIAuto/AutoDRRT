...
public:
    ...
    int DoInfer_1(const camsData &cam_data, float &cost_time);
    int DoInfer_2(const camsData &cam_data, std::vector<Box> &out_detections, float &cost_time);

    nvtype::half* get_bevfeat() { return img_bevfeat_nvhalf; }
...
private:
    ...
    uchar* src_imgs_dev;
    half* img_bevfeat_half;
    nvtype::half* img_bevfeat_nvhalf;
