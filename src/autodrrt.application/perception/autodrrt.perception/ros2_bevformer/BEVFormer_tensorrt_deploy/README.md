...
```shell
export MMDEPLOY_DIR=/home/orin/bevformer_env/mmdeploy/
...
mkdir -p build
cd build
cmake -DCMAKE_CXX_COMPILER=/usr/bin/g++ -DMMDEPLOY_TARGET_BACKENDS=trt -DTENSORRT_DIR=${TENSORRT_DIR} -DCUDNN_DIR=${CUDNN_DIR} ..
make -j$(nproc) 
make install
```
...
cmake .. -DCMAKE_TENSORRT_PATH=/usr/src/tensorrt
make -j$(nproc)
make install
```
