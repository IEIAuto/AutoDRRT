...
from ..remote_machine import SshMachine
from ..utilities import remote_local_util
...
class ExecuteLocal(Action):
    ...
    def execute(self, context: LaunchContext) -> Optional[List[LaunchDescriptionEntity]]:
        ...
        for s in cmd[1:]:
            if s.startswith('__node:='):
                node_name = s[8:]
            if s.startswith("__ns:="):
                name_space = s[6:]
                ns_flag = 1
    
        if ns_flag == 1:
            if name_space=="/":
                name = name_space + node_name
            else:    
                name = name_space + '/' + node_name
        else:
            name = '/' + node_name
        
        flag, machine, redundant_result, restart_result, restart_flag = remote_local_util.remote_local_select(name)

        # print("case is ---!",remote_local_util.case_in_restart_list('/sensing/lidar/top/pointcloud_preprocessor/velodyne_node_container'))
        ...
        try:
            ...
            print("----restart---flag----",restart_flag)
            if restart_flag == 1 and restart_result != 0:
                print("----restart---node----",self.cmd)
                # print("env-----",restart_result['env'])
                info = {'ip': restart_result['ip'], 'username':restart_result['username'],'cpu_affinity':''}
                remote = SshMachine(hostname="inspur", env='source ' + restart_result['env'], info=info, cpu_affinity=None)
                print("remote",remote)
                context.asyncio_loop.create_task(
                    remote.execute_process(
                        process_event_args=self.__process_event_args,
                        ...
                        context=context))
            if flag == 0 and restart_flag == 0:#local
                print("local----")
                context.asyncio_loop.create_task(self.__execute_process(context))
            elif restart_flag == 0:#remote normal
                print("remote--normal")
                context.asyncio_loop.create_task(
                 machine.execute_process(
                    process_event_args=self.__process_event_args,
                    log_cmd=self.__log_cmd,
                    emulate_tty=self.__emulate_tty,
                    shell=self.__shell,
                ...
                if redundant_result!=0:#remote redundant
                    print("redundant_result====",redundant_result)
                    info = {'ip': redundant_result['ip'], 'username':redundant_result['username'],'cpu_affinity':''}
                    # print("----redundant_result---node----",info)
                    remote = SshMachine(hostname="inspur", env='source ' + redundant_result['env'], info=info,  cpu_affinity=None)
                    print("remote",remote)
                    context.asyncio_loop.create_task(
                        remote.execute_process(
                            process_event_args=self.__process_event_args,
