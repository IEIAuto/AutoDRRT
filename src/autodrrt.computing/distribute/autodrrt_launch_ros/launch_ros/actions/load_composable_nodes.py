...
from launch.utilities import remote_local_util
...
class LoadComposableNodes(Action):
    ...
    def execute(
        ...
        if is_a_subclass(self.__target_container, ComposableNodeContainer):
            print("~~__final_target_container_name~~ is ",self.__target_container.node_name)
            ...
            return
        #add restart_info into consider
        #0 -- no restart info
        #1 -- has restart_info and name is as seem as __final_target_container_name
        #2 -- has restart info but name not this container
        case_index = remote_local_util.case_in_restart_list(self.__final_target_container_name)
        print("self.__final_target_container_name",self.__final_target_container_name,"case_index",case_index)
        if case_index < 2:
